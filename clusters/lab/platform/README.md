...
- **It cannot take a hostname away from the old server.** Every name that
  ...
  `prowlarr`, `deluge` and `bazarr` are each a `CNAME` to the apex, verified
  by `dig` (`bazarr` is easy to miss: it has a cluster Ingress like the
  others below, but is still one of the old server's hand-made CNAMEs); a
  random name under the zone returns `NXDOMAIN`, so **there is no
